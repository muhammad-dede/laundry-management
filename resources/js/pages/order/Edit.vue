...
const form = useForm({
    ...
    order_detail:
        props.order?.order_details?.map((item) => ({
            ...
            service_unit_type: item.service?.unit_type,
            service_estimated_days: item.service?.estimated_days,
...
const submit = () => {
    ...
    form.transform((data) => ({
        ...
        estimated_finish_date: estimatedFinishDate
            ...
            : null,
    })).put(route("order.update", props.order.id));
};

const breadcrumbs = [
    { title: "Transaksi", href: route("order.index") },
    { title: "Edit Transaksi", href: route("order.edit", props.order?.id) },
];
</script>
...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <AppMain>
            ...
            >
                <h2 class="text-lg md:text-xl font-bold">Edit Transaksi</h2>
            </div>
