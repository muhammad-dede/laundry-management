...
const submit = () => {
    ...
    form.transform((data) => ({
        ...data,
    })).post(route("pickup.store"));
};

const breadcrumbs = [
    { title: "Pengambilan", href: route("pickup.index") },
    { title: "Tambah Pengambilan", href: route("pickup.create") },
];
</script>

<template>
    <Head title="Tambah Pengambilan" />
    <AppLayout :breadcrumbs="breadcrumbs">
        <AppMain>
            <div
                class="flex flex-col md:flex-row md:justify-between md:items-center gap-4"
            >
                <h2 class="text-lg md:text-xl font-bold">Tambah Pengambilan</h2>
            </div>
